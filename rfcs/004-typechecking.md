...
also enforces this property dynamically. For example, the following function is
...
TypeScript. Then, taking an argument of type `Dyn` and type casing it (using
...
Krishnaswami](https://arxiv.org/abs/1908.05839). Surprisingly, it doesn't seem
to have been deeply studied (outside of the subtyping relation induced by
polymorphism). For subtyping induced by polymorphism, there are different
trade-offs: Dunfield and Krishnaswami restricts their system to impredicative
polymorphism, which is then order-independent [^3]. On the other side, Haskell
...
QuickLook phase that is able to infer some impredicative instantiations.
...
also be `Dyn` (and in this case, given the second argument, it should!). On fix
could be to make all type constructors invariant also for the subtyping relation
generated by `Dyn`, but that may prove quite restrictive in practice.
...
- Polymorphism and subtyping interact in a non trivial way. It is not obvious
   how to apply QuickLook in the presence of our subtyping.

Given that impredicativity looks complex to combine with subtyping, and isn't
really a strong motivation for a language like Nickel, a natural choice is to
...
are way easier and safer to add than to remove. In consequence, we propose to
take a more restrictive approach first, which still open to further pursing one
or the other road if there is a need for more.
